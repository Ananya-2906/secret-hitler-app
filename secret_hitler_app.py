...
st.set_page_config(page_title="Secret Hitler Role Assignment", layout="centered")
...
if not st.session_state.roles:
    ...
    if st.button("🎲 Generate Roles"):
        ...
        st.success("Roles generated! Pass the device to each player to view their secret role.")

if st.session_state.roles:
    st.header("🕵️ Player Role Viewer")
    for i in range(len(st.session_state.roles)):
        ...
        if st.session_state.revealed_player == i:
            ...
            if role == "Fascist":
                teammates = [j + 1 for j, r in enumerate(st.session_state.roles) if r in ["Fascist", "Hitler"] and j != i]
                st.info(f"Your teammates are Player(s): {', '.join(map(str, teammates))}")
            elif role == "Hitler":
